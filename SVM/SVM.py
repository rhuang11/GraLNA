...
for year_test in range(2003, 2015):
    np.random.seed(0)
    print(f"==> Running SVM (training period: 1991-{year_test-2}, testing period: {year_test}, with 2-year gap)...")
    ...
    # Test model
    label_predict = svm.predict(X_test)
    dec_values = svm.predict_proba(X_test)[:, 1]
...
for C in [0.01, 0.1, 1, 10, 100]:
    np.random.seed(0)
    print(f"==> Validating SVM-C{C} (training period: 1991-{year_valid-2}, validating period: {year_valid}, with 2-year gap)...")
    ...
    # Validate model
    label_predict = svm.predict(X_valid)
    dec_values = svm.predict_proba(X_valid)[:, 1]

    # Print validation results
    metrics = evaluate(y_valid, label_predict, dec_values, 0.01)
    print(f"Regularization Parameter C: {C} ==> AUC: {metrics['auc']:.4f}")
    results_tuning.append((C, metrics['auc']))

# Optionally save the tuning results to a file
tuning_df = pd.DataFrame(results_tuning, columns=['C', 'auc'])
tuning_df.to_csv('tuning_svm.csv', index=False)
